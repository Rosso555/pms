          ...
          <form class="form" role="form" action="{$admin_file}?task=staff_info&amp;action=edit&amp;id={$edit.id}" method="post" enctype="multipart/form-data">
            ...
            <form class="form" role="form" action="{$admin_file}?task=staff_info&amp;action=add" method="post" enctype="multipart/form-data">
              ...
              <div class="row">
                  ...
                  <div class="col-md-6">
                      <div class="form-group">
                        <label>{if $multiLang.text_email}{$multiLang.text_email}{else}No Translate (Key Lang:text_email){/if}:</label><span class="text-danger"> *{if $error.email eq 1}{if $multiLang.text_please_input}{$multiLang.text_please_input}{else}No Translate (Key Lang:text_please_input){/if} {if $multiLang.text_email}{$multiLang.text_email}{else}No Translate (Key Lang:text_email){/if}.{/if}</span>
                        <input type="text" class="form-control" name="email" value="{if $edit.email}{$edit.email}{else}{if $smarty.session.staff_info.email|escape}{$smarty.session.staff_info.email|escape}{/if}{/if}" />
                      </div>
                  </div>
              </div>
              <div class="row">
                  ...
                  <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <select class="form-control select2" name="staff_role" style="width:100%">
                          ...
                          <option value="{$v.id}" {if $edit.staff_role_id}{if $edit.staff_role_id eq $v.id} selected{/if}{else}{if $smarty.session.staff_info.staff_role eq $v.id}selected{/if}{/if}>{$v.name}</option>
                          {/foreach}
                        </select>
                      </div>
                  </div>
