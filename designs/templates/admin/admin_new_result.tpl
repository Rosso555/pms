...
<ul class="breadcrumb">
  ...
  <li><a id="bCrumbTest" href="{$admin_file}?task=test">{if $multiLang.text_mailer_lite}{$multiLang.text_mailer_lite}{else}No Translate(Key Lang: text_test){/if}</a></li>
  <li {if $smarty.get.action neq edit}class="active"{/if}>{if $multiLang.text_new_result}{$multiLang.text_new_result}{else}No Translate(Key Lang: text_new_result){/if}</li>
...
<div class="panel panel-primary">
  <div class="panel-heading"><h4 class="panel-title"> {if $multiLang.text_new_result}{$multiLang.text_new_result}{else}No Translate(Key Lang: text_new_result){/if}</h4></div>
  <div class="panel-body">
        ...
        <form class="form-inline" role="form" action="{$admin_file}?task=result" method="GET" style="padding: 1px 0px 12px 1px;">
          ...
          <div class="form-group">
            <button class="btn btn-primary collapsed" type="button" data-toggle="collapse" data-target="#demo" aria-expanded="false" aria-controls="collapseExample">
              <i class="fa fa-plus-circle"></i> {if $multiLang.button_add_new_result}{$multiLang.button_add_new_result}{else}No Translate(Key Lang: button_add_new_result){/if}
            </button>
          </div>
        </form>
        <div id="demo" class="collapse {if $error or $getNResultByID.id} in {/if}">
          {if $getNResultByID.id}
          <form action="{$admin_file}?task=new_result&amp;action=edit&amp;tid={$smarty.get.tid}&amp;id={$getNResultByID.id}" method="post">
          {else}
          <form action="{$admin_file}?task=new_result&amp;action=add&amp;tid={$smarty.get.tid}" method="post">
          {/if}
            <input type="hidden" name="tid" value="{$smarty.get.tid}">
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <select class="form-control select2" name="topic" style="width:100%">
                    ...
                    <option value="{$data.topic_id}" {if $smarty.session.newresult.topic}{if $smarty.session.newresult.topic eq $data.id}selected{/if}{else}{if $getNResultByID.topic_id eq $data.topic_id}selected{/if}{/if}>{$data.name}</option>
                    {/foreach}
                  </select>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  {/if}
                  <input type="text" class="form-control" name="view_order" placeholder="Example: 123..." value="{if $smarty.session.newresult.view_order}{$smarty.session.newresult.view_order}{elseif $getNResultByID.view_order}{$getNResultByID.view_order}{/if}" onkeyup="NumAndTwoDecimals(event , this);">
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  {/if}
                  <textarea class="form-control" name="message" rows="5" id="comment">{if $smarty.session.newresult.message}{$smarty.session.newresult.message}{elseif $getNResultByID.message}{$getNResultByID.message}{/if}</textarea>
                </div>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  {if $getNResultByID.id}
                    <input type="hidden" name="nrid" value="{$getNResultByID.id}" />
                    ...
                    <a href="{$admin_file}?task=new_result&amp;tid={$smarty.get.tid}" class="btn btn-danger" style="color: white;"><i class="fa fa-close"></i> {if $multiLang.button_cancel}{$multiLang.button_cancel}{else}No Translate(Key Lang: button_cancel){/if}</a>
                  {else}
    ...
    <div class="table-responsive">
      <table class="table table-striped">
        ...
        {if $listNewResult|@count gt 0}
        <tbody>
        {foreach from = $listNewResult item = data key=k}
          <tr>
            <td>{$data.name}</td>
            <td>{$data.message}</td>
            <td>{$data.view_order}</td>
            <td><span class="badge">{$data.testQueCount}</span></td>
            <td>
              <a href="{$admin_file}?task=new_result&amp;action=edit&amp;tid={$smarty.get.tid}&amp;id={$data.id}" class="btn btn-success btn-xs" data-toggle1="tooltip" data-placement="top" title="{if $multiLang.button_edit}{$multiLang.button_edit}{else}No Translate(Key Lang: button_edit){/if}"><i class="fa fa-edit"></i></a>
                ...
                <div class="modal-dialog">
                  ...
                  <div class="panel panel-primary modal-content">
                    ...
                    <div class="modal-body">
                      <p>{if $multiLang.text_delete_new_result}{$multiLang.text_delete_new_result}{else}No Translate(Key Lang: text_delete_new_result){/if}
                         <b>({$data.name|escape})</b> ?</p>
                    </div>
                    <div class="modal-footer">
                      <a href="{$admin_file}?task=new_result&amp;action=delete&amp;tid={$smarty.get.tid}&amp;id={$data.id}" class="btn btn-danger btn-md" style="color: white;">
              ...
              <!-- Modal -->
              <a href="{$admin_file}?task=test_question_condition&amp;tid={$smarty.get.tid}&amp;tpid={$data.topic_id}&amp;nrid={$data.id}" class="btn btn-success btn-xs" data-toggle1="tooltip" data-placement="top" title="{if $multiLang.button_add_test_question_condition}{$multiLang.button_add_test_question_condition}{else}No Translate(Key Lang: button_add_test_question_condition){/if}"><i class="fa fa-plus-circle" aria-hidden="true"></i></a>
            </td>
          </tr>
        {/foreach}
        </tbody>
