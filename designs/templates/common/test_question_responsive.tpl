...
{/block}
{block name="title"}{if $getTestById.test_title}{$getTestById.test_title}{/if} {if $getTestById.title}{$getTestById.title}{/if}{/block}
{block name="main"}
<div class="inbox-top">
...
<div class="inbox-test sansserif" style="margin-bottom: 20px;">
  {if $mode eq 'psychologist'}
  <form class="form" action="{$psychologist_file}?task=test_question_psychologist&amp;action=add&amp;tid={$smarty.get.tid}&amp;psy_id={$smarty.get.psy_id}&amp;id={$smarty.get.id}" method="post">
  {/if}
  {if $mode eq 'patient'}
  <form class="form" action="{$patient_file}?task=test_question&amp;tid={$smarty.get.tid}&amp;id={$smarty.get.id}" method="post">
  {/if}
...
$(document).ready( function()
{
  ...
  //Load Check test Question Hide/Show
...
function save_draft()
{
  ...
  {if $mode eq 'psychologist'} var test_psy_id = {$smarty.get.id}; {/if}
  {if $mode eq 'patient'} var test_pat_id = {$smarty.get.id}; {/if}
  ...
  var paramdata = { test_id : test_id,
                    ...
                    {if $mode eq 'psychologist'}test_psy_id : test_psy_id,{/if}
                    {if $mode eq 'patient'}test_pat_id : test_pat_id,{/if}
  ...
  if(test_que_data.length > 0)
  {
    $.ajax({
      type: "POST",
      url: "{if $mode eq 'psychologist'}{$psychologist_file}{/if}{if $mode eq 'patient'}{$patient_file}{/if}?task=test_save_draft",
      dataType:'json',
      data: paramdata,
      success: function(data){
        ...
        {if $mode eq 'patient'} $("#unCompleted").text(data.unCompleted); {/if}
...
function testQuestionHideShow()
{
  {foreach from=$testQuestionHideShowCondition item=v}
    //Declare varriable
    {foreach from=$v.testque_con item=va}
      {if $va.ans_view_order}
      var tqhid_{$v.id}tqid_{$va.test_question_id} = {$va.ans_view_order};
      {else}
      var tqhid_{$v.id}tqid_{$va.test_question_id} = '';
      {/if}
    {/foreach}

    {foreach from=$v.testque_con item=va}
      var check_value = document.getElementsByClassName('check_value{$va.test_question_id}');
      if(check_value.length > 0)
      {
        for (var j = 0; j < check_value.length; j++)
        {
          if(check_value[j].checked == true && check_value[j].disabled == false)
          {
            tqhid_{$v.id}tqid_{$va.test_question_id} = $('#answer_value{$va.test_question_id}_'+check_value[j].value).val();
          }
        }//End for loop
      }//End check value
    {/foreach}
    ...
    {if $v.re_conditional}
      if({$v.re_conditional})
      {
        ...
        $('#row_tque{$v.test_question_id}').show("slow");
        {if $v.is_required eq 1}
          {if $v.type eq 1 OR $v.type eq 2}
            //Text free field
            var text_free = $('#test{$v.test_question_id}').val();
            //Check condition
            if(text_free != '') val_text_free.push({ val: text_free });

            if(val_text_free.length == 0)
            {
              $('#text_is_required_{$v.test_question_id}').val(1);
              $('#test{$v.test_question_id}').attr('required', true);
            }
          {/if}

          {if $v.type eq 3}
            //Radio
            var val_chk = document.getElementsByClassName('check_value{$v.test_question_id}');
            //Radio
            if(val_chk.length > 0)
            {
              for (var j = 0; j < val_chk.length; j++)
              {
                if(val_chk[j].checked == true)
                {
                  val_checked.push({ val: val_chk[j].value });
                }
              }//End for loop
            }//End check value
            ...
            {
              $('#is_required{$v.test_question_id}').val(1);
              {foreach from=$v.answer item=ans}
                $('#radio{$ans.id}_{$v.test_question_id}').attr('required', true);
              {/foreach}
            }
          {/if}
          ...
          {if $v.type eq 4}
            //Check Box
            var val_chk_box = document.getElementsByClassName('check_box_value{$v.test_question_id}');
            //Check Box
            if(val_chk_box.length > 0)
            {
              for (var j = 0; j < val_chk_box.length; j++)
              {
                if(val_chk_box[j].checked == true)
                {
                  val_check_box.push({ val_ckh_boox: val_chk_box[j].value });
                }
              }//End for loop
            }//End check value

            if(val_check_box.length == 0)
            {
              $('.chk_box_is_required{$v.test_question_id}').val(1);
              {foreach from=$v.answer item=ans}
                $('#checkbox{$ans.id}_{$v.test_question_id}').attr('required', true);
              {/foreach}
            }
          {/if}
        {/if}//end check is required

      } else {
        //Radio
        var check_value = document.getElementsByClassName('check_value{$v.test_question_id}');
        //Check Box
        var check_box_value = document.getElementsByClassName('check_box_value{$v.test_question_id}');
        //Text free field
        var text_free = $('#test{$v.test_question_id}').val();

        $('#row_tque{$v.test_question_id}').hide();

        {if $v.is_required eq 1}
          {if $v.type eq 1 OR $v.type eq 2}
            $('#text_is_required_{$v.test_question_id}').val(0);
            $('#test{$v.test_question_id}').removeAttr("required");
          {/if}

          {if $v.type eq 3}
            $('#is_required{$v.test_question_id}').val(0);
            {foreach from=$v.answer item=ans}
              $('#radio{$ans.id}_{$v.test_question_id}').removeAttr("required");
            {/foreach}
          {/if}

          {if $v.type eq 4}
            $('.chk_box_is_required{$v.test_question_id}').val(0);
            {foreach from=$v.answer item=ans}
              $('#checkbox{$ans.id}_{$v.test_question_id}').removeAttr("required");
            {/foreach}
          {/if}
        ...
        if(check_value.length > 0)
        {
          for (var j = 0; j < check_value.length; j++)
          {
            if(check_value[j].checked == true)
            {
              {if $v.type eq 3}
                $('#tq_id{$v.test_question_id}').attr('disabled', true);
                $('#rais_email{$v.test_question_id}').attr('disabled', true);
                $('#raanswer_id{$v.test_question_id}').attr('disabled', true);
                $('#racontent{$v.test_question_id}').attr('disabled', true);
                {foreach from=$v.answer item=ans}
                  $('#radio{$ans.id}_{$v.test_question_id}').attr("checked", false);
                {/foreach}
              {/if}
            }
          }//End for loop
        ...
        {
          for (var j = 0; j < check_box_value.length; j++)
          {
            if(check_box_value[j].checked == true)
            {
              {if $v.type eq 4}
                {foreach from=$v.answer item=ans}
                  //Action button uncheck
                  $('#chk_tqid{$v.test_question_id}{$ans.id}').attr('disabled', true);
                  $('#is_email{$v.test_question_id}{$ans.id}').attr('disabled', true);
                  $('#answer_id{$v.test_question_id}{$ans.id}').attr('disabled', true);
                  $('#content{$v.test_question_id}{$ans.id}').attr('disabled', true);
                  $('#checkbox{$ans.id}_{$v.test_question_id}').attr("checked", false);
                {/foreach}
              {/if}
            }
          }//End for loop
        ...
        if(text_free != '')
        {
          $('#tq_id_{$v.test_question_id}').attr('disabled',true);
          $('#answer_id_{$v.test_question_id}').attr('disabled', true);
          $('#content_{$v.test_question_id}').attr('disabled', true);
          $('#is_email_{$v.test_question_id}').attr('disabled', true);
          $('#test{$v.test_question_id}').val('');
        }
      }
    {/if}
  {/foreach}
