  ...
  {if $reponseAnswerByTestPsyt|@COUNT gt 0}
    ...
    {foreach from=$reponseAnswerByTestPsyt item=v}
      {if $v.type eq 3 OR $v.type eq 4}
        <div class="col-md-6 col-sm-12 col-xs-12 response-ans-col">
          ...
          {elseif $v.type eq 4}
            ...
            <div class="checkbox" style="margin-left: 16px;">
              <label><input type="checkbox" {if $ans.re_answer_id}checked{/if}><span style="line-height: 1.2;">{$ans.title}</span></label>
            </div>
            {/foreach}
          {/if}
        </div>
