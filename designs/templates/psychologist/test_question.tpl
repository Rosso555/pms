...
<ul class="breadcrumb">
  ...
  <li class="active">{if $multiLang.text_test_question}{$multiLang.text_test_question}{else}No Translate(Key Lang: text_test_question){/if}</li>
</ul>

<div class="inbox-top">
  <h3 class="text-center">{$getTestById.test_title}</h3>
...
<div class="inbox-test sansserif" style="margin-bottom: 20px;">
    ...
    {if $result|@COUNT gt 0}
      ...
      {foreach item = v from=$result key=id name=first_foo}
        {if $v.flag eq 1}
          <tr>
            <td colspan="{$v.answer|@COUNT + 1}">
              <p style="margin-top: 20px; margin-bottom: -10px;"><b> {$v.g_answer_title}</b></p>
            </td>
          </tr>
          <tr>
            <td>&nbsp;</td>
        ...
        {else}
          <tr>
            <td colspan="{if $v.answer}{$v.answer|@COUNT + 1}{else}{$totalAnswer + 1}{/if}">
              <p style="margin-top: 20px; {if $v.hide_title eq 1}margin-bottom: 0px;{/if}">{if $v.is_required eq 1}<span style="color:red;" id="required_{$v.id}">*</span><b>{/if} {if $v.hide_title eq 0}{$v.que_title}{else}{$v.description}{/if}</b></p>
              {if $v.hide_title eq 0}<span style="margin-left: 16px;">{$v.description}</span><br>{/if}
              ...
              {if $v.type eq 1}
                <p style="margin-left: 16px; margin-top: 15px;">
                  {if $v.is_required eq 1}<input type="hidden" name="is_required[]" id="text_is_required{$v.id}" value="{$v.is_required|escape}"/>{/if}
                  <!-- hidden test_question_id -->
                  <input type="hidden" id="tq_id{$v.id}" name="tq_id[]" value="{$v.test_question_id|escape}" disabled>

                  <input type="hidden" id="is_email{$v.id}" name="is_email[]" value="{if $v.is_email eq 1}1{else}0{/if}" disabled>
                  <input type="hidden" id="answer_id{$v.id}" name="answer_id[]" value="NULL" disabled>
                  <input type="hidden" id="content{$v.id}" name="content[]" value="{foreach from=$sessionContent item=va}{if $va.id eq $ans.id}{$va.content}{/if}{/foreach}" disabled>
...
function checkvalue(e, field, id) {
  {foreach item = v from= $result key=id}
    ...
    if($('#test{$v.id}').val() !== '') $('#tq_id{$v.id}').removeAttr('disabled'), $('#text_is_required{$v.id}').val(0);
    if($('#test{$v.id}').val() == '')  $('#tq_id{$v.id}').attr('disabled','disabled'), $('#text_is_required{$v.id}').val(1);

    if($('#test{$v.id}').val() !== '') $('#answer_id{$v.id}').removeAttr('disabled');
    if($('#test{$v.id}').val() == '') $('#answer_id{$v.id}').attr('disabled','disabled');

    if($('#test{$v.id}').val() !== '') $('#content{$v.id}').removeAttr('disabled');
    if($('#test{$v.id}').val() == '') $('#content{$v.id}').attr('disabled','disabled');

    if($('#test{$v.id}').val() !== '') $('#is_email{$v.id}').removeAttr('disabled');
    if($('#test{$v.id}').val() == '') $('#is_email{$v.id}').attr('disabled','disabled');

    if($('#test{$v.id}').val() !== '') $('#content{$v.id}').val(text);
